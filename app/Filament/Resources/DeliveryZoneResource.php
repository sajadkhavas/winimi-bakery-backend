        ...
        return $form->schema([
            Forms\Components\Section::make('محدوده')
                ->schema([
                    ...
                    Forms\Components\TextInput::make('city')
                        ...
                        ->maxLength(100)
                        ->dehydrateStateUsing(fn (?string $state): ?string => filled($state) ? trim($state) : null),
                    Forms\Components\TextInput::make('priority')->label('اولویت')->numeric()->minValue(0)->default(100)->required(),
            ...
            Forms\Components\Section::make('روش‌ها و هزینه‌ها')
                ->schema([
                    ...
                    Forms\Components\TextInput::make('standard_fee_toman')->label('هزینه عادی')->numeric()->minValue(0)->default(0)->suffix(' تومان'),
                    Forms\Components\Toggle::make('chilled_enabled')->label('ارسال سرد'),
                    Forms\Components\TextInput::make('chilled_fee_toman')->label('هزینه سرد')->numeric()->minValue(0)->default(0)->suffix(' تومان'),
                    Forms\Components\Toggle::make('pickup_enabled')->label('تحویل حضوری'),
                    Forms\Components\TextInput::make('pickup_fee_toman')->label('هزینه حضوری')->numeric()->minValue(0)->default(0)->suffix(' تومان'),
                    Forms\Components\TextInput::make('packaging_fee_toman')->label('هزینه بسته‌بندی')->numeric()->minValue(0)->default(0)->suffix(' تومان'),
                    Forms\Components\TextInput::make('free_delivery_threshold_toman')->label('ارسال رایگان از مبلغ')->numeric()->minValue(0)->nullable()->suffix(' تومان'),
                ])->columns(4),
            Forms\Components\Section::make('محدودیت عملیات')
                ->schema([
                    Forms\Components\TextInput::make('minimum_order_toman')->label('حداقل سفارش')->numeric()->minValue(0)->nullable()->suffix(' تومان'),
                    Forms\Components\TextInput::make('preparation_min_days')
                        ->label('حداقل روز آماده‌سازی')
                        ->numeric()
                        ->minValue(0)
                        ->default(0)
                        ->rules(['lte:preparation_max_days']),
                    Forms\Components\TextInput::make('preparation_max_days')
                        ->label('حداکثر روز آماده‌سازی')
                        ->numeric()
                        ->minValue(0)
                        ->default(0)
                        ->rules(['gte:preparation_min_days']),
                    Forms\Components\TextInput::make('daily_order_limit')->label('ظرفیت روزانه')->numeric()->minValue(1)->nullable(),
                ])->columns(4),
        ]);
    }
