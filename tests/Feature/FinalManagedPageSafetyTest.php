...
class FinalManagedPageSafetyTest extends TestCase
{
    ...
    public function test_core_managed_pages_are_protected_from_route_and_delete_drift(): void
    {
        ...
        $this->assertMatchesRegularExpression(
            "/TextInput::make\('slug'\).*?->disabled\(fn \(\?BakeryContentPage \$record\): bool => \$record !== null && self::isProtectedPage\(\$record\)\).*?->dehydrated\(fn \(\?BakeryContentPage \$record\): bool => \$record === null \|\| ! self::isProtectedPage\(\$record\)\)/s",
            $source,
        );
